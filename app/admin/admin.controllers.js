...
adminModule.controller("adminMainController", [
    "$scope",
    "$translate",
    "adminOUGSFactory",
    "adminUGFactory",
    ...
    function (
        ...
        $scope.UGList = adminUGFactory.get_UG.query(function (response) {
            adminUGFactory.get_UG_set.query(
                function (response) {
                    if (response.value) {
        ...
        $scope.OUGSList = adminOUGSFactory.get_OUGS.query(function (response) {
            adminOUGSFactory.get_OUGS_set.query(
                function (response) {
                    if (response.value) {
        ...
        $scope.submitUG = function () {
            var payload =
                '{"value":"' + $scope.selectedUG.name + '", "object":' + JSON.stringify($scope.selectedUG) + "}";
        ...
        $scope.submitOUGS = function () {
            var payload =
                '{"value":"' + $scope.selectedOUGS.id + '", "object":' + JSON.stringify($scope.selectedOUGS) + "}";
        ...
        $scope.submitDS = function () {
            adminDSFactory.get_DS_set
                .query($scope.selectedDS)
                .$promise.then(
                    function (response1) {
        ...
        $scope.submitIG = function () {
            adminIGFactory.get_IG_set
                .query($scope.selectedIG)
                .$promise.then(
                    function (response1) {
        ...
        $scope.submitDossierConfigComplete = function () {
            ...
            var payload =
                $scope.dossierConfigComplete.UG &&
                $scope.dossierConfigComplete.OUGS &&
                $scope.dossierConfigComplete.OUG &&
                $scope.dossierConfigComplete.A;
            ...
            adminDossierConfigCompleteFactory.get_dossierConfigComplete_set.query().$promise.then(
                function (response1) {
                    ...
                    console.log(
                        "adminModule: dossierConfigComplete: update, old - ",
                        response.value,
                        " / new - ",
                        payload.value
                    );
                    if (response.value != payload.value) {
                        return adminDossierConfigCompleteFactory.upd_dossierConfigComplete.query(
                            payload,
                            function (response2a) {
                                window.location.reload(true);
                            }
                ...
                function () {
                    return adminDossierConfigCompleteFactory.set_dossierConfigComplete.query(
                        payload,
                        function (response2b) {
