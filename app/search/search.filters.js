    ...
    return function (text, phrase_glo, phrase_col, name) {
        if ((phrase_glo || (phrase_col && name)) && text) {
            if (phrase_glo) {
                ...
                mots_glo = mots_glo.filter(function (n) {
            ...
            if (phrase_col && name && phrase_col[name]) {
                ...
                mots_col = mots_col.filter(function (n) {
                    return n != undefined && n != "";
                });
                mots_col = mots_col.join("|");
            }
            text = text.replace(
                new RegExp("(" + mots_glo + "|" + mots_col + ")", "gi"),
    ...
    return function (data, phrases) {
        var temp = (data || []).filter(function (row) {
            ...
            keys.forEach(function (key) {
                ...
                mots = mots.map(function (s) {
                    return s.trim();
                });
                mots = mots.filter(function (n) {
                    return n != undefined && n != "";
                });
                //console.log(mots);
